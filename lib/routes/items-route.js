...
.post('/:id/addstore', bodyParser, ensureRole('owner'), (req, res, next) => {
    return Item.findById(req.params.id)
        .then(item => {
            item.stores.push(req.body.store_id);
...
.post('/:id/removestore', ensureRole('owner'), bodyParser, (req, res, next) => {
    return Item.findById(req.params.id)
        .then(item => {
            const newItem = item.stores.filter(x => {
                if (x != req.body.store_id) {
                    return x;
                } else {
                    return null;
                }
